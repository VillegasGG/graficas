...
archivo_excel = 'compare_results_1_roots.xlsx'
...
df_k1_long = df_k1.melt(var_name='Root Degree', value_name='Execution Time (s)')
df_k1_long['Algorithm'] = 'Rollout (k=1)'

df_k2_long = df_k2.melt(var_name='Root Degree', value_name='Execution Time (s)')
df_k2_long['Algorithm'] = 'Rollout (k=2)'

df_k3_long = df_k3.melt(var_name='Root Degree', value_name='Execution Time (s)')
df_k3_long['Algorithm'] = 'Rollout (k=3)'

df_greedy_long = df_greedy.melt(var_name='Root Degree', value_name='Execution Time (s)')
df_greedy_long['Algorithm'] = 'Greedy'

df_all = pd.concat([df_k1_long, df_k2_long, df_k3_long, df_greedy_long], ignore_index=True)

df_all['Root Degree'] = pd.to_numeric(df_all['Root Degree'])
...
sns.lineplot(data=df_all, x='Root Degree', y='Execution Time (s)', hue='Algorithm',
